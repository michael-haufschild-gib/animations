...
export function ProgressBarsCelebrationBurst() {
  ...
  const triggerMilestone = useCallback(
    (index: number, markerContainer: HTMLElement, marker: HTMLElement) => {
        ...
        registerTimeout(() => {
          const anim = ring.animate(
            [
              ...
              { transform: 'scale(2.0)', opacity: '0.6', offset: 0.3 },
    ...
    [registerAnimation, registerTimeout]
  )
    ...
    (
      ...
      if (latest < previous - 1) {
        ...
        milestonePositions.forEach((pos, index) => {
          const progressPercent = latest * 100
          const milestonePercent = pos * 100

          if (
            !reachedMilestonesRef.current.has(index) &&
            previous * 100 < milestonePercent &&
            progressPercent >= milestonePercent
          ) {
            reachedMilestonesRef.current.add(index)
            const milestoneData = milestoneMarkers.find((m) => m.index === index)
            if (milestoneData) {
              triggerMilestone(index, milestoneData.container, milestoneData.marker)
            }
          }
        })
      }
  ...
  const animateProgress = useCallback(
    (
      ...
      const easeInOut = (t: number): number => {
        // cubic-bezier(0.4, 0.0, 0.2, 1) approximation
  ...
  useEffect(() => {
    ...
    const startAnimation = () => {
      ...
      milestonePositions.forEach((pos, index) => {
        ...
        markerContainer.className = 'animation-element milestone-container'
        ...
        marker.style.transition = 'all 0.25s cubic-bezier(0.34, 1.56, 0.64, 1)'
      ...
      currentAnimation = animateProgress(
        0,
        1,
        4000,
        (progress) => {
          if (!stopped) {
            ...
            updateProgress(progress, milestoneMarkers)
          }
        },
        () => {
          if (!stopped) {
