import{useEffect,useRef,useState}from"react";import"./ProgressBarsZoomedProgress.css";/**
 *
 */export function ProgressBarsZoomedProgress(){const[level,setLevel]=useState(1);const[levelPoints,setLevelPoints]=useState([0,0,0]);const[levelReached,setLevelReached]=useState([true,false,false]);const animationRef=useRef<ReturnType<typeof setTimeout>|null>(null);useEffect(()=>{let currentLevel=1;let currentLevelPoints=[0,0,0];const update=()=>{// Calculate random points (1 to level-1, minimum 1)
...
const progress1Width=levelPoints[0]/3*100;const progress2Width=levelPoints[1]/6*100;return<div className="pf-zoomed-progress"data-animation-id="progress-bars__zoomed-progress">
      ...
      <div className="pf-zoomed-progress__track"style={{left:`${getTrackPosition()}%`}}>
        ...
        {/* Level 1 marker */}
        <div className={`pf-zoomed-progress__level pf-zoomed-progress__level--one ${levelReached[0]?"reached":""}`}>
          <span>1</span>
        </div>

        {/* Level 2 marker */}
        <div className={`pf-zoomed-progress__level pf-zoomed-progress__level--two ${levelReached[1]?"reached":""}`}>
          <span>2</span>
        </div>
        ...
        {/* Level 3 marker */}
        <div className={`pf-zoomed-progress__level pf-zoomed-progress__level--three ${levelReached[2]?"reached":""}`}>
          <span>3</span>
        </div>
      </div>
