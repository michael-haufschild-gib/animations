...
describe('animationRegistry', () => {
    ...
    it('should have correct metadata for each group', () => {
      Object.values(categories).forEach((category) => {
        Object.values(category.groups).forEach((group) => {
          ...
          expect(group.metadata.title).toBeTruthy() // Optional fields
  ...
  describe('animation metadata validation', () => {
      ...
      Object.values(categories).forEach((category) => {
        Object.values(category.groups).forEach((group) => {
          ...
          Object.entries(group.framer).forEach(([animId, animation]) => {
            ...
            expect(animation.component).toBeDefined() // Component can be a function or an object (lazy-loaded React component)
      ...
      Object.values(categories).forEach((category) => {
        Object.values(category.groups).forEach((group) => {
          ...
          Object.values(group.framer).forEach((animation) => {
            ...
            expect(Array.isArray(animation.metadata.tags)).toBe(true) // Optional field
    ...
    it('should have unique animation IDs within each code mode (framer and css separately)', () => {
      ...
      expect(framerAnimationIds.length).toBe(uniqueFramerIds.size) // Check CSS animations for uniqueness
  ...
  describe('buildRegistryFromCategories()', () => {
    ...
    it('should produce a flat map of animation ID to component', () => {
      ...
      Object.entries(registry).forEach(([id, component]) => {
        ...
        expect(id).toBeTruthy() // Component can be a function or an object (lazy-loaded React component)
    ...
    it('should include animations from both framer and css (may have ID collisions)', () => {
      const registry = buildRegistryFromCategories() // Note: Framer and CSS animations may share IDs, so the registry size
    ...
    it('should map animation IDs to components from the hierarchy', () => {
      const registry = buildRegistryFromCategories() // When IDs collide between framer and css, the last one wins
