 ...
 */ export function CollectionEffectsCoinMagnet() {
  ...
  useEffect(() => {
    ...
    setCoins(generatedCoins) // Clean up coins after animation completes
  ...
  const getTranslation = (startX: number, startY: number) => {
    ...
    const containerHeight = containerRef.current.offsetHeight // Target is at right: 40px, top: 40px from container edges
    const targetX = containerWidth - 40
    const targetY = 40 // Start position in pixels
    const startXPx = (startX / 100) * containerWidth
    const startYPx = (startY / 100) * containerHeight // Calculate delta
        ...
        {coins.map((coin) => {
          ...
          return (
            <m.div
              ...
              initial={{ x: '-50%', y: '-50%', scale: prefersReducedMotion ? 0.8 : 1, rotate: 0, opacity: prefersReducedMotion ? 0 : 0.8 }}
              animate={
                prefersReducedMotion
                  ? { x: `calc(-50% + ${translation.x}px)`, y: `calc(-50% + ${translation.y}px)`, scale: [0.8, 1, 1, 0.8], opacity: [0, 1, 1, 0] }
                  : { x: `calc(-50% + ${translation.x}px)`, y: `calc(-50% + ${translation.y}px)`, scale: [1, 1.2, 0.8], rotate: [0, coin.rotation * 0.5, coin.rotation], opacity: [0.8, 1, 1, 0] }
              }
