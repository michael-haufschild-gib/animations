 ...
 */ export function ModalCelebrationsConfettiPulse() {
  const pulses = useMemo(() => Array.from({ length: 3 }, (_, i) => ({ id: i, delay: i * 0.18, color: confettiColors[i % confettiColors.length], duration: 1.4 })), [])
  const confettiParticles = useMemo(
    () =>
      Array.from({ length: 20 }, (_, i) => {
        ...
        const duration = 0.78
        return { id: i, color: confettiColors[i % confettiColors.length], tx, ty, rot, delay, duration }
      }),
    []
  )
  return (
      ...
      <div className="pf-celebration__layer">
        ...
        {pulses.map((pulse) => (
          <m.span
            ...
            transition={{ duration: pulse.duration, delay: pulse.delay, ease: easeOut }}
          />
        ))}
        ...
        {confettiParticles.map((particle) => (
          <m.span
            ...
            style={{ left: '50%', top: '55%', background: particle.color }}
            initial={{ x: 0, y: 0, scale: 0.6, rotate: 0, opacity: 0 }}
            animate={{ x: particle.tx, y: particle.ty, scale: 1, rotate: particle.rot, opacity: [0, 1, 1, 0] }}
            transition={{ duration: particle.duration, delay: particle.delay, ease: easeOut, times: [0, 0.2, 0.8, 1] }}
          />
        ))}
      </div>
    </div>
