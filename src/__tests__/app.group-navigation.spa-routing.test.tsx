...
describe('App routing behavior', () => {
  it('navigates to selected group via router without full-page reload', async () => {
  ...
  it('switches between framer and css variants via code mode switch', async () => {
    const user = userEvent.setup()
    renderRoutedApp('/alpha-framer')
    ...
    await user.click(screen.getByRole('button', { name: /Open menu/i }))
    await user.click(screen.getByRole('button', { name: 'Framer' }))
