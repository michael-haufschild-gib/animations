...
Task: Implement an animation as requested by the user. Ask questions if requirements are unclear. Follow the animation principles outlined below - no exceptions. Make sure you follow folder structure as explained in CLAUDE.md - if you don't know where to put the animation, ask the user.
...
Let motion follows natural curved paths rather than rigid straight lines. Also for many other animation typs prefer curved value paths over linear value paths.
...
**Principle 9: Keep Web and Native Close Togehter**
